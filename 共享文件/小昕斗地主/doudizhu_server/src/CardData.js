...
CardData.shuffle = function () {
    function getRandomArrayElements(arr, count) {
        ...
        return [shuffled.slice(min), shuffled.slice(0, min)];
    }

    let rst = [];
    let newCard = CardData.getNewCard();
    let [one, rst1] = getRandomArrayElements(newCard, 17);
    let [two, rst2] = getRandomArrayElements(rst1, 17);
    let [three, extra] = getRandomArrayElements(rst2, 17);
    rst.push(one, two, three, extra);
    return rst;
}
